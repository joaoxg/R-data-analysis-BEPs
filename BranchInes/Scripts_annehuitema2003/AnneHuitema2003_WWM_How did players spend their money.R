...
folder_name <- "housinggame_session_19_250923_EPA_IntroDays_Overasselt" ##Change here the datafolder you want to put into a set
...
#first three lines to # the plot 
# interactive_plot <- ggplotly(plot)
# htmlwidgets::saveWidget(interactive_plot, "interactive_plot.html")
# browseURL("interactive_plot.html")
ggsave("output.png", plot = plot, width = 12, height = 6, dpi = 300) #changed plot = plot_name into plot = plot
plot(plot)
...
tryCatch({
  # Define new output directory inside Scripts_annehuitema2003
  output_dir <- paste0(script_dir_up, "/Scripts_annehuitema2003/data_output")
  
  # Create folder if it doesn't exist
  dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)
  
  # Full output path
  output_file <- paste0(output_dir, "/Income_dist_", folder_name, ".xlsx")
  
  # Write the Excel file
  write_xlsx(list_income_dist, output_file)
  
  message("File written successfully to: ", output_file)
}, error = function(e) {
  message("Error: ", e$message)
})


#BElow Juliettes code putting the income dist to her output folder. Above linking it to mine outpur folder. 
# tryCatch({
