...
# -----------------------------
# Read classes lookup table
# -----------------------------
classes_lookup <- read_excel(classes_file) %>%
  transmute(
    player_code = as.character(Q_PlayerNumber),
    classes     = as.character(class)   # voeg als 'classes' toe
  )
...
sheets_list <- lapply(sheet_names, function(sh) {
  ...
  # Alleen aanpassen als player_code bestaat
  if ("player_code" %in% names(df)) {
    # Zorg dat join key hetzelfde type is
    df2 <- df %>%
      mutate(player_code = as.character(player_code)) %>%
      left_join(classes_lookup, by = "player_code")

    # Garandeer: classes komt als LAATSTE kolom, verder niets wijzigen
    df2 <- df2 %>%
      select(setdiff(names(df2), "classes"), classes)

    return(df2)
  } else {
    # Sheet zonder player_code blijft exact zoals die is
    return(df)
  }
})
...
# -----------------------------
# Write back (overwrite dezelfde Excel) met identieke sheets
# -----------------------------
write_xlsx(sheets_list, improv_file)

message("Done: 'classes' column added (only where player_code exists) to ", improv_file)
